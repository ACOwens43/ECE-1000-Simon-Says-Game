...
def play_again_prompt():
    oled.fill(0)
    oled.text("Play Again?", 0, 0)
    oled.text("Press 1: Yes", 0, 16)
    oled.text("Press 2: No", 0, 32)
    oled.show()
    print("Waiting for Play Again choice...")
    while True:
        if button_pins["red"].value() == 1:  # Button 1 for Yes
            return True
        if button_pins["green"].value() == 1:  # Button 2 for No
            return False
...
while True:
    ...
    if not play_again_prompt():
        oled.fill(0)  # Turn off the display
        oled.show()
